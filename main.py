...
T1_in = 20
T2_in = 60
T1_out = 31.43 #for now, need to iterate in order to find the exact temperatures
T2_out = 47.84 #for now, need to iterate in order to find the exact temperatures
Y = 0.014
...
def find_H(m_dot_1, m_dot_2, N_tube, N_b):
     A_tube = (np.pi/4)*(d_i)**2
     A_pipe = (np.pi/4)*d_sh**2
     m_tube = m_dot_2/N_tube
     ...
     a = 0.34 #this is a constant, 0.2 for triangular arrangement, 0.34 for square TODO
     Re_sh = rho*V_sh*d_sh_1/mu
     del_p_sh = 4*a*Re_sh**(-0.15)*N_tube*rho*V_sh**2
...
def Q_lmtd(m_dot_1, m_dot_2, N_tube, N_b):
     H = find_H(m_dot_1, m_dot_2, N_tube, N_b)
     ...
     A = N_tube*np.pi*d_i*L_tube 
     A = N_tube*np.pi*d_i*L_tube*F
     Q_dot_LMTD = H*A*del_T_lm
     
     return Q_dot_LMTD
     
def Q_entu(m_dot_1, m_dot_2, N_tube, N_b):
     H = find_H(m_dot_1, m_dot_2, N_tube, N_b)
...
def many_configs():
     ...
     for i,N_tubes in enumerate(N_tubes_array):
          for j,N_baffles in enumerate(N_baffles_array):
               Q_dots_ENTU[i,j] = Q_entu(m_dot_1, m_dot_2, N_tubes, N_baffles)
               Q_dots_LMTD[i,j] = Q_lmtd(m_dot_1, m_dot_2, N_tubes, N_baffles)
...
def one_config():
     ...
     N_tubes = 13
     N_baffles = 9
     print(Q_entu(m_dot_1, m_dot_2, N_tubes, N_baffles))
     print(Q_lmtd(m_dot_1, m_dot_2, N_tubes, N_baffles))
